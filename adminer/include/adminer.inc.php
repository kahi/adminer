...
<?php
			...
			if ($_GET["ns"] !== "" && !$missing && DB != "") {
				echo '<div class="add-table"><a href="' . h(ME) . 'create="' . bold($_GET["create"] === "") . ">" . lang('Create new table') . "</a></div><p>\n";
				$tables = tables_list();
				if (!$tables) {
...
<?php
	...
	*/
	function tablesPrint($tables) {
		...
		foreach ($tables as $table => $type) {
			echo '<li' . bold($_GET["table"] == $table || $_GET["select"] == $table) . '><a href="' . h(ME) . 'select=' . urlencode($table) . '" class="d' . bold_class($_GET["select"] == $table) . '">' . $this->tableName(array("Name" => $table)) . "</a> ";
			echo '<a href="' . h(ME) . 'table=' . urlencode($table) . '" class="s' . bold_class($_GET["table"] == $table). '" title="' . lang('Show structure') . '">' . $this->tableName(array("Name" => $table)) . "</a></li>\n"; //! Adminer::tableName may work with full table status
		}
		echo "</ul>\n";
	}
